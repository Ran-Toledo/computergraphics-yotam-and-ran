...
void DrawImguiMenus()
{
	...
	if (ImGui::Checkbox("Sky Box", &skyBoxActivated))
	{
	...
	{
		ImGui::Begin("Cameras Menu", nullptr, ImGuiWindowFlags_AlwaysAutoResize);
		...
		//float cameraAt[3] = { 0.0f, 0.0f, 0.0f };
		...
		//float worldCameraRotation[3] = { 0.0f, 0.0f, 0.0f };
		...
		if (ImGui::SliderFloat3("Translation", cameraEye, 0.0f, 100.0f, "%.3f"))
		...
		//ImGui::SliderFloat3("At", cameraAt, 0.0f, 5.0f, "%.3f");
		if (ImGui::SliderFloat3("Rotation", localCameraRotation, 0.0f, 180.0f, "%.3f"))
			scene->GetActiveCamera().ApplyTransform(glm::vec3(cameraEye[0], cameraEye[1], cameraEye[2]), glm::vec3(localCameraRotation[0], localCameraRotation[1], localCameraRotation[2]));
		//ImGui::SliderFloat3("World Rotation", worldCameraRotation, 0.0f, 180.0f, "%.3f");
		ImGui::SetNextItemWidth(173);
		if (ImGui::SliderFloat("Z Near", &zNear, 0.f, 1.f, "%.3f"))
			scene->GetActiveCamera().SetNear(zNear);

		ImGui::SameLine();
		ImGui::SetNextItemWidth(173);
		if (ImGui::SliderFloat("Z Far", &zFar, 30.0f, 800.0f, "%.3f"))
		...
		if (ImGui::Button("Reset Camera Transformations"))
		{
			cameraEye[0] = 0.0f;	cameraEye[1] = 0.0f;	cameraEye[2] = -1.0f;
			//cameraAt[0] = 0.0f;		cameraAt[1] = 0.0f;		cameraAt[2] = 0.0f;
			localCameraRotation[0] = 0.0f;	localCameraRotation[1] = 0.0f;	localCameraRotation[2] = 0.0f;
			//worldCameraRotation[0] = 0.0f; worldCameraRotation[1] = 0.0f;	worldCameraRotation[2] = 0.0f;
			fov = 50.0f;
			zNear = 30.0f;
			zFar = 150.0f;
		}
	...
	ImGui::Begin("Models Menu", nullptr, ImGuiWindowFlags_AlwaysAutoResize);
	ImGui::PushItemWidth(400);
	...
	{
		std::ostringstream s;
		s << "Model " << i << " - " << scene->GetModel(i).GetModelName();
		std::string mystring = s.str();
	...
	if (scene->GetModelCount() != 0 && current_item)
	{
		...
		if (transformation_type == types[0])
		{
			...
			/*if (ImGui::Button("Rotate X 45 deg"))
				localRotations[0] += 45.0f;
			ImGui::SameLine();
			if (ImGui::Button("Rotate Y 45 deg"))
				localRotations[1] += 45.0f;
			ImGui::SameLine();
			if (ImGui::Button("Rotate Z 45 deg"))
				localRotations[2] += 45.0f;*/

			if (ImGui::Button("Reset Local Transformations"))
			{
				//localScales[0] = 10.0f; localScales[1] = 10.0f; localScales[2] = 10.0f;
				//localRotations[0] = 0.0f; localRotations[1] = 0.0f; localRotations[2] = 0.0f;
				//localTranslations[0] = 0.0f; localTranslations[1] = 0.0f; localTranslations[2] = 0.0f;
			}
		}
		else
		{
			...
			/*if (ImGui::Button("Rotate X 45 deg"))
				worldRotations[0] += 45.0f;
			ImGui::SameLine();
			if (ImGui::Button("Rotate Y 45 deg"))
				worldRotations[1] += 45.0f;
			ImGui::SameLine();
			if (ImGui::Button("Rotate Z 45 deg"))
				worldRotations[2] += 45.0f;*/

			if (ImGui::Button("Reset World Transformations"))
			{
				//worldScales[0] = 1.0f; worldScales[1] = 1.0f; worldScales[2] = 1.0f;
				//worldRotations[0] = 0.0f; worldRotations[1] = 0.0f; worldRotations[2] = 0.0f;
				//worldTranslations[0] = 0.0f; worldTranslations[1] = 0.0f; worldTranslations[2] = 0.0f;
			}
		}

		/*ImGui::Checkbox("Display bounding box", &scene.GetActiveModel().display_bounding_box);
		ImGui::SameLine();
		ImGui::Checkbox("Display normals", &scene.GetActiveModel().display_normals);
		ImGui::SameLine();
		ImGui::SetNextItemWidth(100);
		ImGui::InputFloat("Normals scale", &scene.GetActiveModel().normals_scale, 1);*/

		ImGui::Begin("Color/Texture Menu", nullptr, ImGuiWindowFlags_AlwaysAutoResize);
		...
		if (activeModel.IsTextureActivated())
		{
			...
			ImGui::SameLine();
			if (ImGui::RadioButton("Cylindrical", cylindricalTextureActivated))
				activeModel.SwitchToCylindrical();
			ImGui::SameLine();
			if (ImGui::RadioButton("Spherical", sphericalTextureActivated))
	...
	{
		std::shared_ptr<MeshModel> cube = Utils::LoadMeshModel("..\\Data\\cube.obj");
		cube->ScaleModel(lightCubeFactor);
		scene->AddLight(std::make_shared<PointLight>(PointLight(*cube, { glm::vec3(1, 1, 1), glm::vec3(1, 1, 1), glm::vec3(1, 1, 1) })));
	}
	ImGui::SameLine();
	...
	if (scene->GetLightCount() > 0)
	{
		...
		if (pointLight = std::dynamic_pointer_cast<PointLight>(light)) // It is point Light
		{
			...
			if (scene->GetLightCount() != 0)
			{
				...
				if (light_transformation_type == light_types[0])
				{
					...
					if (ImGui::SliderFloat3("Translation", lightLocalTranslation, 0.0f, 10.f, "%.3f"))
					{
					...
					if (ImGui::Button("Reset Local Transformations"))
					{
						lightLocalScale[0] = 1.0f; lightLocalScale[1] = 1.0f; lightLocalScale[2] = 1.0f;
						lightLocalRotation[0] = 0.0f; lightLocalRotation[1] = 0.0f; lightLocalRotation[2] = 0.0f;
						lightLocalTranslation[0] = 0.0f; lightLocalTranslation[1] = 0.0f; lightLocalTranslation[2] = 0.0f;
					}
				}
				else
				{
					...
					if (ImGui::SliderFloat3("Translation", lightWorldTranslation, 0.0f, 10.f, "%.3f"))
					{
					...
					if (ImGui::Button("Reset World Transformations"))
					{
						lightWorldScale[0] = 1.0f; lightWorldScale[1] = 1.0f; lightWorldScale[2] = 1.0f;
						lightWorldRotation[0] = 0.0f; lightWorldRotation[1] = 0.0f; lightWorldRotation[2] = 0.0f;
						lightWorldTranslation[0] = 0.0f; lightWorldTranslation[1] = 0.0f; lightWorldTranslation[2] = 0.0f;
					}
				}
			}
		}
		else
		{
			...
			//light.GetDirection() = glm::vec3(direction[0], direction[1], direction[2]);
		}
